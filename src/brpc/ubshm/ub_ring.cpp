...
static RETURN_CODE UbrScheduleClearTimer(UbrTrx *trx, void* (*cb)(void*),
    ...
    // cleanup_ctl is plain pointer storage, accessed through a butil::atomic
    // view so that scheduling and force close compete on a single RMW.
    butil::atomic<UbrCleanupCtl*>* ctl_slot =
        reinterpret_cast<butil::atomic<UbrCleanupCtl*>*>(&trx->cleanup_ctl);
    if (ctl_slot->load() != nullptr) {
    ...
    if (!ctl_slot->compare_exchange_strong(expected, ctl)) {
    ...
    if (!UBRingManager::TryPublishUnitCleanupCtl(trx->trx_mgr_index,
        ...
        expected = ctl;
        ctl_slot->compare_exchange_strong(expected, nullptr);
    ...
    if (BAIDU_UNLIKELY(rc != UBRING_OK)) {
        ...
        if (ATOMIC_COMPARE_EXCHANGE_STRONG(ctl->state, state_expected, UBR_CLEANUP_RUNNING)) {
            ...
            ctl_slot->compare_exchange_strong(published, nullptr);
    ...
    if (BAIDU_UNLIKELY(ATOMIC_LOAD(trx->ubr_id) != ctl->ubr_id)) {
        ...
        UbrCleanupCtl* published = ctl;
        if (!ctl_slot->compare_exchange_strong(published, nullptr)) {
            published = nullptr;
        }
        UBRingManager::DetachUnitCleanupCtl(trx->trx_mgr_index, ctl);
        ctl->ReleaseRef();                   // starter reference
